...
All implementation geometry is named in `tokens.css`. The responsive thresholds are `--breakpoint-compact: 1200px` and `--breakpoint-narrow: 640px`; `AppShell` reads those CSS tokens and exposes `data-layout="wide|compact|narrow"` on both the shell and `body`, so in-shell and portal/overlay selectors share one production breakpoint state. Other named geometry includes `--nav-current-rule`, `--nav-item-leading`, `--project-context-wide-min/max`, `--project-context-compact-min/max`, `--stage-wide-min`, `--stage-compact-min`, `--stage-step-min`, `--stage-line-thickness`, `--master-wide`, `--master-compact`, `--cast-workflow-wide`, `--cast-workflow-modal`, `--home-continue-cover-column`, `--home-continue-cover-width`, `--home-continue-next`, `--home-continue-action`, `--home-row-cover-column`, `--home-row-cover-width`, `--home-row-status`, `--home-row-action`, `--home-row-overflow`, `--dialog-new-project-source-compact`, `--new-project-cover-width`, `--new-project-cover-compact`, `--script-speaker-width`, `--script-direction-min/max`, `--produce-character-min`, `--produce-character-compact`, `--produce-direction-width`, `--produce-duration-width`, `--produce-audio-width`, `--produce-audio-compact`, `--produce-state-width`, `--produce-state-compact`, `--export-cover-width`, `--export-publication-ratio`, `--export-chapters-ratio`, `--export-output-ratio`, `--player-track-min`, `--player-timeline-wide-min`, `--player-context-min/max`, `--player-time-width`, `--player-blur`, `--player-volume-width`, `--cast-section-block`, `--cast-section-space`, `--cast-heading-gap`, and `--showcase-min`. Raw pixel dimensions belong only in the token definition file.

At 1536×1024 project mode the rail spans the full viewport height; the 80px player begins at x224/y944 and never covers the rail. The header begins at x224 and workspace begins at x224/y104. Global mode uses the same rail and an 88px title-and-action header. Navigation rows are 40px high with 20px icons, 14/20 semibold labels, 8px radius, 16px rail inset, and 24px group gaps. The rail always presents Project (Home, Script, Cast, Produce, Export), Library (Library, Voices, Templates), and Settings (Settings, More); the current project is named in the project header rather than repeated in a sidebar card.

At 1024×768, components reflow; nothing is scaled. At 390×844, the DOM/reading order remains navigation → header → main → player. The rail becomes a single 61px horizontal navigation strip with a sticky Alexandria mark, an optional sticky Read-only badge, and horizontally scrollable labelled destinations; it does not become a multi-row wall above the page. The main uses 16px horizontal/20px vertical padding, toolbars wrap, master/detail becomes sequential, dialogs use the viewport with 16px insets, and the player remains one full-width region. No separate mobile mockup or hidden duplicate tree exists.

### Page geometry and ownership anchors

- Cast wide master/detail: `400px minmax(0, 1fr)` in one bordered surface with an internal divider and no redundant page-title band. The visible `Characters` heading is the route h1. Compact: `280px minmax(0, 1fr)`. Narrow: list then selected detail. Specialist Cast workflows mount in the existing shell overlay as a maximum 760px drawer. Identity-review dialogs use the 900px modal token so the review form and its guarded action remain visible without a nested horizontal squeeze. Below 640px either surface uses the viewport width; neither replaces the Cast route or creates a legacy workspace.
...
| `NavRail` | Always-visible Project Home/Script/Cast/Produce/Export; Library/Voices/Templates; Settings/More. Default, hover, focus, current, long label. Current uses a 3px terracotta rule plus quiet ivory fill and `aria-current`; no duplicate current-project card. Narrow mode is one contained horizontal strip with sticky brand/read-only context and no document-width overflow. |
| `GlobalHeader` | 88px wide/compact, responsive narrow, and owns the single visible route h1, subtitle, controls, and at most one page primary. Global routes do not repeat the same title in an in-page band. Loading/error retain stable geometry. |
...
| `PageTitleBlock` | Project-stage or embedded-drawer title block only: one focused h1, optional concise subtitle, stable title-to-content band; 48/52 wide and 40/44 compact. Global routes use `GlobalHeader` instead of duplicating a second title. Cast uses `Characters` as its visible route h1 inside the master pane. |
...
| `ResizableAudioColumns` | Produce exposes persistent token-driven separators with a 32px hit area. Pointer drag resizes; Left/Right adjust by the resize-step token; Home restores the token default. Separators use `role="separator"`, vertical orientation, and current/min/max pixel values. Saved widths are temporary-profile preferences; the default composition keeps section actions and Status visible. |
...
`PersistentPlayer` is the single full transport. States are absent, inactive, active, loading, playing, paused, and failed. Both inactive and active states occupy the approved 80px shell band; inactive keeps all controls visibly disabled and names what must be selected, while active provides back-ten-seconds, one tactile play, forward-ten-seconds, timecodes, chapter context, volume, queue, and overflow.
...
- Projects/New Project/Script/Library/Voices/Templates own their direct page DOM. Global pages expose one visible h1 in the shell header and retain only a hidden route-ownership marker inside the destination. Voices is read-only library context, not a second assignment source.
- Cast owns one character list and one selected profile. The default profile is read-only and Voice changes require explicit Edit Voice mode. Order is identity → dominant Voice → reference/transcript → approved preview → Character → Appearance → Advanced. Character, Appearance, and Advanced are collapsed by default. Persona Visual expands inside Appearance; it is never a global page or second list.
- Produce owns audio rows and selected-chunk inspector from Spec + phase3c. Selection is independent of Stale/Current/etc.; only rows with available audio receive play controls and only rows with a valid action receive an action menu. Script and Produce collections page responsively: 30 rows narrow, 60/75 compact, and 120/150 wide, with selected-row pinning and explicit Load More.
...
- Settings is a global preferences destination that renders only the selected section. Wide mode uses a side section index; compact/narrow use one horizontal section strip. Maintenance is a separate More destination; deep links preserve return context and focus the requested subsection without embedding or moving its page.
...
Focus follows visible DOM order. Route changes focus the visible h1. List selection retains row focus and announces detail context. Settings deep links render one selected section, place it below the global header and section chooser, and focus its visible h2; Back/Forward restores exact route context. Dialogs/drawers trap and restore focus. `prefers-reduced-motion: reduce` sets all authored animation/transition duration to 0ms while preserving states and feedback.
