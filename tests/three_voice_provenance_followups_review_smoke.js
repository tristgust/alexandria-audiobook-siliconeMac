...
const REVIEW_ROOT = path.resolve(
  process.env.ALEXANDRIA_PROVENANCE_FOLLOWUPS_REVIEW_ROOT
    || '.omo/evidence/b17-t69-three-voice-provenance-followups/review',
);
const PORT = Number(process.env.ALEXANDRIA_PROVENANCE_FOLLOWUPS_REVIEW_PORT || 18893);
...
async function waitForServer() {
  ...
  throw new Error('Provenance follow-up review server did not start.');
}
...
async function runBrowser(browserType, browserName) {
  ...
  try {
    ...
    assert.match(await page.title(), /Benny \/ Doctor Provenance Salvage/i);
    ...
    await page.locator('#search').fill('inevitable');
    assert.strictEqual(await page.locator('#route-nav button').count(), 1);
    ...
    const saved = await page.evaluate((cardId) => {
      const key = `alexandria:three-voice-provenance-followups:${window.THREE_VOICE_FINAL_SALVAGE_DATA.round_id}`;
      return JSON.parse(localStorage.getItem(key))[cardId];
    }, boundary.card_id);
    assert.strictEqual(saved.decision, 'approve_final');
    ...
    const importPayload = {
      ...
      rows: [{ card_id: separation.card_id, revision: 8, decision: 'candidate_B', notes: 'imported' }],
    };
    ...
    assert.strictEqual(download.suggestedFilename(), 'alexandria_three_voice_provenance_followups_review.json');
...
(async () => {
  ...
  assert.strictEqual(manifest.abandoned_role_contaminated_clip_count, 1);
  ...
  try {
    ...
    const separation = data.rows.find((row) => row.card_type === 'source_separation');
    const ranged = await request(`${BASE_URL}/${separation.candidates[0].audio}`, { Range: 'bytes=0-1023' });
