...
class UserTestInterfaceRepairTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.source = INDEX.read_text(encoding="utf-8")

    def test_dataset_progress_uses_external_readable_label(self) -> None:
        self.assertIn('id="dsb-progress-label"', self.source)
        self.assertIn("`${done} of ${total} complete · ${pct}%${failureCopy}`", self.source)
        self.assertIn("const failed = dsbRows.filter(r => r.status === 'error').length", self.source)
        self.assertIn("wrap.hidden = false", self.source)
        self.assertIn("wrap.hidden = true", self.source)
        self.assertIn(
            'id="dsb-progress-bar" class="progress-bar" role="progressbar"',
            self.source,
        )
        self.assertNotIn(
    ...
    def test_clone_upload_restores_same_open_voice_panel(self) -> None:
        for fragment in (
            "function restoreVoicePanelAfterReload(",
            "const voiceName = originalPanel?.dataset.voice || ''",
            "selectedCloneId: result.voice_id",
            "panel.open = true",
    ...
    def test_reference_audio_button_tracks_play_pause_state(self) -> None:
        for fragment in (
            "setCloneReferenceButtonState",
            "icon.classList.toggle('fa-pause', isPlaying)",
            "audio.addEventListener('play'",
            "audio.addEventListener('pause'",
            "audio.addEventListener('ended'",
            "Pause reference audio",
            "if (activeTab !== 'characters') stopCloneReferenceAudio()",
            "stopCloneReferenceAudio();\n            releaseCharacterVoiceCard();\n            if (!window._voiceBackendCapabilities)",
        ):
            with self.subTest(fragment=fragment):
                self.assertIn(fragment, self.source)

    def test_apple_compute_falls_back_to_real_cpu_activity(self) -> None:
        for fragment in (
            "else if (stats.compute)",
            "stats.compute.utilization_percent",
            "system memory ${stats.compute.system_memory_percent}%",
            "setInterval(updateSystemStats, 3000)",
        ):
            with self.subTest(fragment=fragment):
                self.assertIn(fragment, self.source)

    def test_preparer_exposes_upload_and_download_workflow(self) -> None:
        for fragment in (
            "/api/preparer/upload",
            "/api/preparer/upload/${encodeURIComponent(task.audio_filename)}",
            "async function refreshPreparerOutputs()",
            "Prepared datasets",
            "Download ZIP",
