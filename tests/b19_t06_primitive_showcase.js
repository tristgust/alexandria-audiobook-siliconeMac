...
async function inspectorProbe(session) {
  const open = await session.evaluate(`(() => { const root=getComputedStyle(document.documentElement),inline=innerWidth>=Number.parseFloat(root.getPropertyValue('--breakpoint-inspector')),host=document.querySelector('.showcase-main'); let overlay=document.querySelector('[data-overlay-root]'); if(!overlay){overlay=document.createElement('div');overlay.dataset.overlayRoot='';document.body.append(overlay);} const slot=document.createElement('section'),content=document.createElement('div'),inspector=document.createElement('aside'),trigger=AlexandriaUI.iconButton({name:'chevron',label:'Toggle context inspector',tooltip:''}),body=document.createElement('div'); slot.dataset.shellInspectorSlot=''; slot.dataset.inspectorState='open'; slot.dataset.inspectorProbe=''; content.dataset.inspectorContent=''; content.textContent='Primary working area'; inspector.dataset.shellInspector=''; inspector.dataset.state='open'; inspector.setAttribute('aria-label','Context inspector'); trigger.dataset.inspectorTrigger=''; body.dataset.inspectorBody=''; body.textContent='Context inspector · notes and references'; inspector.append(trigger,body); slot.append(content); host.prepend(slot); const baseline=content.getBoundingClientRect().width; (inline?slot:overlay).append(inspector); const box=inspector.getBoundingClientRect(),after=content.getBoundingClientRect().width,slotWidth=slot.getBoundingClientRect().width,style=getComputedStyle(inspector); slot.scrollIntoView({block:'center'}); return {inline,alias:root.getPropertyValue('--color-action').trim(),primary:root.getPropertyValue('--color-action-primary').trim(),breakpoint:Number.parseFloat(root.getPropertyValue('--breakpoint-inspector')),width:box.width,position:style.position,baseline,after,slotWidth}; })()`);
  await session.screenshot('inspector-open.png');
  const collapsed = await session.evaluate(`(() => { const slot=document.querySelector('[data-inspector-probe]'),inspector=document.querySelector('[data-shell-inspector]'); slot.dataset.inspectorState='collapsed'; inspector.dataset.state='collapsed'; const box=inspector.getBoundingClientRect(),content=slot.querySelector('[data-inspector-content]').getBoundingClientRect(),trigger=inspector.querySelector('[data-inspector-trigger]'),triggerBox=trigger.getBoundingClientRect(); return {width:box.width,contentWidth:content.width,position:getComputedStyle(inspector).position,trigger:Boolean(trigger.getAttribute('aria-label'))&&triggerBox.width>=32&&triggerBox.height>=32,bodyHidden:getComputedStyle(inspector.querySelector('[data-inspector-body]')).display==='none'}; })()`);
  await session.screenshot('inspector-collapsed.png');
  const generic = await session.evaluate(`(() => { const inspector=document.querySelector('[data-shell-inspector]'),overlay=document.querySelector('[data-overlay-root]'); inspector.dataset.state='overlay'; overlay.append(inspector); const box=inspector.getBoundingClientRect(),result={directChild:inspector.parentElement===overlay,width:box.width,position:getComputedStyle(inspector).position}; document.querySelector('[data-inspector-probe]').remove(); inspector.remove(); if(!overlay.id&&!overlay.childElementCount)overlay.remove(); result.cleaned=!document.querySelector('[data-inspector-probe],[data-shell-inspector]'); return result; })()`);
  const openWidth = Math.abs(open.width - 360) <= 1, collapsedWidth = Math.abs(collapsed.width - 40) <= 1;
  return { actionTokenAlias:Boolean(open.alias)&&open.alias===open.primary,inspectorBreakpoint:open.breakpoint===1180,inspectorOpen:openWidth,inspectorCollapsed:collapsedWidth,inspectorCollapsedTrigger:collapsed.trigger&&collapsed.bodyHidden,inspectorPlacement:open.inline?open.position!=='fixed':open.position==='fixed',inspectorContentPreserved:open.inline?open.slotWidth-open.after>=359:Math.abs(open.baseline-open.after)<=1,genericOverlayCompatible:generic.directChild&&generic.position==='fixed'&&Math.abs(generic.width-360)<=1,inspectorCleanup:generic.cleaned,metrics:{open,collapsed,generic} };
}
