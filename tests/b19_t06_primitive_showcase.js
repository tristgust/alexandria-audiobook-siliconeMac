...
async function inspectViewport(baseUrl, artifacts, width, height) {
  ...
  try {
    ...
    const matrix = await matrixProbe(session); const { castOrder: measuredCastOrder, ...matrixAssertions } = matrix; const interaction = await interactionProbe(session); const accessibility = await accessibilityProbe(session);
    ...
    return { key,width,height,status:Object.values(assertions).every(value=>typeof value==='object'||Boolean(value))?'PASS':'FAIL',assertions,missing,metrics:{...metrics,matrix:{...matrix,castOrder:measuredCastOrder}},errorCount:errors.length,screenshot:path.join(viewportArtifacts,'viewport.png'),fullPageScreenshot:full };
