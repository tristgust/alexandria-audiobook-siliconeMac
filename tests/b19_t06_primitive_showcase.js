...
const PLAYER_CONTROLS = ['skip-back', 'play-pause', 'skip-forward', 'volume', 'queue', 'overflow'];
...
async function press(session, key, modifiers = 0) {
  ...
  const event = { key, modifiers, ...definition };
...
async function overlayProbe(session) {
  ...
  result.dirtyConfirmation = await session.evaluate(`['Save changes','Discard changes','Cancel'].every(t=>[...document.querySelectorAll('[data-dirty-confirmation] button')].some(b=>b.textContent.trim()===t))`);
...
async function interactionProbe(session) {
  const result = {};
  await session.evaluate(`document.querySelector('[data-test="disclosure-trigger"]')?.click()`);
  ...
  result.playerTransport = await session.evaluate(`(() => { const player=document.querySelector('.app-shell > [data-primitive="persistent-player"]'); if(!player||typeof player.getPlayerState!=='function')return false; const events=[];player.addEventListener('alexandriaplayerchange',event=>events.push(event.detail));const back=player.querySelector('[data-player-control="skip-back"]'),play=player.querySelector('[data-player-control="play-pause"]'),forward=player.querySelector('[data-player-control="skip-forward"]'),volume=player.querySelector('[data-player-control="volume"]'),queue=player.querySelector('[data-player-control="queue"]'),options=player.querySelector('[data-player-control="overflow"]');back.click();play.click();forward.click();volume.value='0';volume.dispatchEvent(new Event('input',{bubbles:true}));queue.click();const queuePanel=queue.closest('.popover-controller')?.querySelector('.popover'),queueOpened=queue.getAttribute('aria-expanded')==='true'&&!queuePanel?.hidden&&queuePanel?.textContent.includes('No additional audio queued');queue.click();options.click();const speed=[...options.closest('.popover-controller').querySelectorAll('[role="menuitem"]')].find(item=>item.textContent.includes('1.25'));speed?.click();const state=player.getPlayerState();return state.state==='paused'&&state.position===48&&state.volume===0&&state.speed===1.25&&queueOpened&&options.getAttribute('aria-label').includes('1.25×')&&events.map(event=>event.action).join(',')==='skip-back,pause,skip-forward,volume,speed'; })()`);
...
async function matrixProbe(session) {
  return session.evaluate(`(() => {
    ...
    const absent=AlexandriaUI.persistentPlayer({state:'absent'}),inactive=document.querySelector('[data-contract-state="persistent-player:inactive"]'),playerAbsent=absent===null,expectedPlayerHeight=innerWidth<640?112:80,playerInactive=Boolean(inactive&&Math.abs(inactive.getBoundingClientRect().height-expectedPlayerHeight)<=1&&inactive.querySelector('[data-player-control="play-pause"]')?.disabled);
