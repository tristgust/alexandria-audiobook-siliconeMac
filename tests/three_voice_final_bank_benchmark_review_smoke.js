...
const REVIEW_ROOT = path.resolve(
  process.env.ALEXANDRIA_FINAL_BANK_BENCHMARK_REVIEW_ROOT
    || '.omo/evidence/b17-t74-three-voice-final-bank-benchmark/review',
);
const PORT = Number(process.env.ALEXANDRIA_FINAL_BANK_BENCHMARK_REVIEW_PORT || 18895);
...
async function waitForServer() {
  ...
  throw new Error('Final-bank benchmark server did not start.');
}
...
async function runBrowser(browserType, browserName) {
  ...
  try {
    ...
    assert.match(await page.title(), /Final 31-Reference Bank Benchmark/i);
    ...
    assert.strictEqual(JSON.stringify(data.rows).includes('combined_bank'), false);
    ...
    for (const target of ['narrator', 'benny', 'doctor']) {
      await page.locator('#target-filter').selectOption(target);
      assert.strictEqual(await page.locator('#route-nav button').count(), 2);
    }
    await page.locator('#target-filter').selectOption('all');
    await page.locator('#search').fill('fatalistic dread');
    assert.strictEqual(await page.locator('#route-nav button').count(), 1);
    assert.match(await page.locator('#function-label').innerText(), /Fatalistic dread/i);
    ...
    assert.strictEqual(download.suggestedFilename(), 'alexandria_three_voice_final_bank_benchmark_review.json');
...
(async () => {
  ...
  assert.strictEqual(manifest.final_validated_bank_reference_count, 31);
  assert.strictEqual(manifest.export_filename, 'alexandria_three_voice_final_bank_benchmark_review.json');
  ...
  try {
    ...
    const ranged = await request(`${BASE_URL}/${data.rows[0].candidate_A.audio}`, { Range: 'bytes=0-1023' });
