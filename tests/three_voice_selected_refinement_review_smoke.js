...
const REVIEW_ROOT = path.resolve(
  process.env.ALEXANDRIA_SELECTED_REFINEMENT_REVIEW_ROOT
    || '.omo/evidence/b17-t60-three-voice-selected-refinement-review/review',
);
const PORT = Number(process.env.ALEXANDRIA_SELECTED_REFINEMENT_REVIEW_PORT || 18890);
...
async function waitForServer() {
  ...
  throw new Error('Selected-refinement review server did not start.');
}
...
async function runBrowser(browserType, browserName) {
  ...
  try {
    const context = await browser.newContext({ acceptDownloads: true, viewport: { width: 1000, height: 820 } });
    ...
    assert.match(await page.title(), /Selected Source Refinements/i);
    ...
    const data = await page.evaluate(() => window.THREE_VOICE_SELECTED_REFINEMENT_DATA);
    assert.strictEqual(data.rows.length, 2);
    assert.deepStrictEqual(data.rows.map((row) => row.clip_id), ['benny_shock_grief', 'doctor_indomitable_determination']);
    assert.strictEqual(data.rows.every((row) => row.technical_pass === true), true);
    ...
    for (let index = 0; index < 2; index += 1) {
      ...
      decoded.forEach((item) => {
        ...
        assert.ok(item.duration > 1.0, JSON.stringify(item));
        assert.strictEqual(item.channels, 1);
      });
      decodedCount += decoded.length;
    }
    assert.strictEqual(decodedCount, 4);
    ...
    assert.strictEqual(download.suggestedFilename(), 'alexandria_three_voice_selected_refinement_review.json');
...
(async () => {
  ...
  assert.strictEqual(manifest.selected_vs_refined_comparison, true);
  ...
  try {
    ...
    const prefix = 'window.THREE_VOICE_SELECTED_REFINEMENT_DATA = ';
    const data = JSON.parse(dataText.slice(prefix.length, -1));
    const ranged = await request(`${BASE_URL}/${data.rows[0].refined_audio}`, { Range: 'bytes=0-1023' });
    ...
    console.log(JSON.stringify({ candidateCount: 2, rangeStatus: ranged.status, browsers: results, serverErrors }, null, 2));
  } finally {
    server.kill('SIGTERM');
  }
})().catch((error) => {
