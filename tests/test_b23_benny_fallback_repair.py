...
from benchmarks import build_b23_benny_fallback_repair as repair
...
class BennyFallbackRepairTests(unittest.TestCase):
    def project_fixture(self, root: Path) -> Path:
        ...
        config = {
            "BERNICE": {
                "seed": repair.SEED,
                "character_style": "Wry Benny fixture.",
    ...
    def test_build_is_blind_single_choice_and_non_installing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertNotIn("benny_diary_buoyant_confidence", public)
            ...
            self.assertIn("Benny reference", html)
            self.assertNotIn("Doctor reference", html)
            self.assertTrue(
                (output / "review" / "reference" / "benny_reference.wav").is_file()
            )
            self.assertFalse(
                (output / "review" / "reference" / "doctor_reference.mp3").exists()
            )
            answer = json.loads(
