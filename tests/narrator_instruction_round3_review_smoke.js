...
const REVIEW_ROOT = path.resolve(
  process.env.ALEXANDRIA_NARRATOR_INSTRUCTION_ROUND3_REVIEW_ROOT
    || '/Users/tristan/.devspace/worktrees/alexandria-audiobook.git-78fc5814/.omo/evidence/b17-t10-narrator-instruction-round3/review',
);
...
(async () => {
  ...
  let importPath;
  try {
    assert.strictEqual(REVIEW_MANIFEST.candidate_count, 24);
    ...
    assert.deepStrictEqual(REVIEW_MANIFEST.candidate_counts_by_style, {
      neutral: 4,
      grief: 4,
      panic: 4,
      angry: 4,
    ...
    assert.strictEqual(contract.roundId, 'alexandria_narrator_instruction_round3_v1');
    assert.match(contract.title, /Blind Round 3/i);
    ...
    const scoredSampleId = await page.locator('.sample-card').first().getAttribute('data-sample-id');
    ...
    await scoredCard.locator('textarea[data-field="notes"]').fill('Round 3 browser smoke note.');
    await page.waitForTimeout(350);
    const stored = await page.evaluate((sampleId) => {
      const key = Object.keys(localStorage).find((item) => item.startsWith('alexandria:narrator-instruction:round3:') && !item.endsWith(':style'));
      return JSON.parse(localStorage.getItem(key))[sampleId];
    }, scoredSampleId);
    assert.strictEqual(stored.identity_1_to_5, 4);
    assert.strictEqual(stored.notes, 'Round 3 browser smoke note.');
    ...
    importPath = path.join(os.tmpdir(), `alexandria-narrator-round3-import-${process.pid}.json`);
    fs.writeFileSync(importPath, JSON.stringify({
      ...
      rows: [{
        ...
        revision: 999,
        notes: 'Imported Round 3 partial result.',
    ...
    assert.strictEqual(
      ...
      'Imported Round 3 partial result.',
    );
    assert.strictEqual(
    ...
    assert.match(download.suggestedFilename(), /narrator_instruction_round3_style_grief/);
    await download.cancel();
    downloadPromise = page.waitForEvent('download', { timeout: 10000 });
    await page.locator('#export-all').click();
    download = await downloadPromise;
    assert.match(download.suggestedFilename(), /narrator_instruction_round3_cumulative_all/);
    await download.cancel();
    ...
    console.log(JSON.stringify({
      ...
      candidatesPerStyle: contract.styleCounts.neutral,
      scoredSampleId,
      importSampleId,
