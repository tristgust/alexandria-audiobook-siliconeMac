...
const REVIEW_ROOT = path.resolve(
  process.env.ALEXANDRIA_FINAL_SALVAGE_REVIEW_ROOT
    || '.omo/evidence/b17-t57-three-voice-final-salvage-review/review',
);
const PORT = Number(process.env.ALEXANDRIA_FINAL_SALVAGE_REVIEW_PORT || 18889);
...
async function waitForServer() {
  ...
  throw new Error('Final salvage review server did not start.');
}

async function decodeVisibleAudio(page) {
  return page.evaluate(async () => {
    const results = [];
    for (const element of [...document.querySelectorAll('audio')].filter((audio) => !audio.closest('section').hidden)) {
...
async function runBrowser(browserType, browserName) {
  ...
  try {
    const context = await browser.newContext({ acceptDownloads: true, viewport: { width: 1100, height: 850 } });
    ...
    assert.match(await page.title(), /Final Three-Voice Salvage Gate/i);
    ...
    assert.deepStrictEqual(data.card_type_counts, { boundary_final: 5, source_separation: 4 });
    assert.strictEqual(JSON.stringify(data).match(/BS-RoFormer|MelBand|MDX-Net|model_bs|gabox|Voc_FT/), null);
    ...
    assert.strictEqual(decodedCount, 26);

    await page.locator('#type-filter').selectOption('source_separation');
    assert.strictEqual(await page.locator('#route-nav button').count(), 4);
    ...
    await page.locator('#search').fill('bittersweet nostalgia');
    ...
    const firstCard = data.rows[0].card_id;
    ...
    const saved = await page.evaluate((cardId) => {
      const key = `alexandria:three-voice-final-salvage:${window.THREE_VOICE_FINAL_SALVAGE_DATA.round_id}`;
    ...
    const importPayload = {
      ...
      rows: [{ card_id: boundary.card_id, revision: 8, decision: 'still_wrong', notes: 'imported' }],
    };
    ...
    assert.strictEqual(download.suggestedFilename(), 'alexandria_three_voice_final_salvage_review.json');
...
(async () => {
  ...
  assert.strictEqual(manifest.card_count, 9);
  assert.deepStrictEqual(manifest.card_type_counts, { boundary_final: 5, source_separation: 4 });
  ...
  try {
    ...
    const sample = data.rows[0].candidates[0].audio;
    ...
    console.log(JSON.stringify({ candidateCount: manifest.card_count, rangeStatus: ranged.status, browsers: results, serverErrors }, null, 2));
  } finally {
    server.kill('SIGTERM');
  }
})().catch((error) => {
