...
class AudioArtifactDocumentationTests(unittest.TestCase):
    ...
    def test_document_exists_and_states_content_bound_audio_contract(self) -> None:
        ...
        for phrase in (
            "content-bound production artifact",
            "audio_state",
            "audio_fingerprint",
            "audio_sha256",
            "Legacy `status: done` audio",
    ...
    def test_document_requires_atomic_install_and_final_output(self) -> None:
        for phrase in (
            ...
            "Audacity ZIP",
            "M4B",
            "preserves the last successful canonical",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, self.doc)
    ...
    def test_document_requires_operation_scoped_exact_audio_rollback(self) -> None:
        for phrase in (
            ...
            "backup path",
            "hard rollback conflict",
            "restores the pre-operation JSON and audio bytes",
            "audio/<sha256>.bin",
            "stored once per operation",
        ):
            with self.subTest(phrase=phrase):
                self.assertIn(phrase, self.doc)
    ...
    def test_open_work_is_not_misrepresented_as_complete(self) -> None:
        for phrase in (
            ...
            "older live invalidation records",
            "reference-bank",
            "adapter",
            "alias-target",
            "voice-save",
