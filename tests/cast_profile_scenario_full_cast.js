...
async function runFullCastScenario({ assertions, details, server, session }) {
  ...
  assertions.fullCastTasksVisible = details.fullCastTasks.title === 'Full Cast and identity tasks'
  ...
  details.completeCastBundle = await session.evaluate(`(() => {
    ...
    return {
      ...
      primary:panel.querySelector('[data-export-complete-cast]')?.textContent.trim()||'',
  ...
  assertions.completeCastBundleVisible = details.completeCastBundle.checked === 3
    ...
    && details.completeCastBundle.primary === 'Export Cast bundle'
  ...
  await session.waitFor(`Boolean(document.querySelector('[data-complete-cast-bundle] a[href*="/api/tasks/"]'))`);
  ...
  await session.waitFor(`Boolean(document.querySelector('[data-full-cast-task="roster_discovery"] a[href*="/api/tasks/"]'))`);
