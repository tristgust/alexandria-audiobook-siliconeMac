...
DEFAULT_REVIEW = (
    ROOT / "benchmarks" / "b23_benny_fallback_repair_20260804_user_decision.json"
)
DEFAULT_ANSWER_KEY = Path(
    "/Users/tristan/Downloads/b23_benny_fallback_repair_20260804/"
    "answer-keys/answer-key.json"
)
DEFAULT_OUTPUT = (
    ROOT / "benchmarks" / "b23_benny_fallback_repair_20260804_decision.json"
)
PROJECT_DECISION_FILENAME = "voice_route_listening_decisions.json"
EVIDENCE_PATH = ".omo/evidence/b23-benny-fallback-repair-20260804.json"
TARGET_ROUTE_KEY = "benny_criminal_sardonic_concern"
LEGACY_PREFIX = "DFR"
CURRENT_PREFIX = "BFR"
ANCHOR_REFERENCE_ROUTES = {
    "b18_neutral_identity_anchor": "neutral",
    "b18_current_route_anchor": TARGET_ROUTE_KEY,
}


class BennyFallbackAdjudicationError(RuntimeError):
    pass
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise BennyFallbackAdjudicationError(f"Could not read {label}: {exc}") from exc
    if not isinstance(value, dict):
        raise BennyFallbackAdjudicationError(f"{label} must contain an object.")
    return value
...
def _validate_review(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(value) != expected or value.get("schema_version") != 1:
        raise BennyFallbackAdjudicationError("Benny fallback review has an invalid schema.")
    if value.get("round_id") != ROUND_ID:
        raise BennyFallbackAdjudicationError("Benny fallback review has the wrong round ID.")
    selection = str(value.get("selection") or "").strip()
    if not selection:
        raise BennyFallbackAdjudicationError("Benny fallback review has no selection.")
    return {
        "schema_version": 1,
        "round_id": ROUND_ID,
...
def _normalize_selection(selection: str, answer_by_sample: Mapping[str, Any]) -> str:
    if selection in answer_by_sample or selection.casefold() == "none":
        return selection
    if selection.startswith(LEGACY_PREFIX):
        ordinal = selection[len(LEGACY_PREFIX) :]
        replacement = f"{CURRENT_PREFIX}{ordinal}"
        if replacement in answer_by_sample:
            return replacement
    raise BennyFallbackAdjudicationError(
        f"Unknown Benny fallback selection: {selection!r}."
    )
...
def adjudicate(
    *,
    ...
    review = _validate_review(_read_json(review_file, "Benny fallback review"))
    answer_key = _read_json(answer_file, "Benny fallback answer key")
    if answer_key.get("round_id") != ROUND_ID:
        raise BennyFallbackAdjudicationError("Benny fallback answer key has the wrong round ID.")

    answers = answer_key.get("answers")
    if not isinstance(answers, list):
        raise BennyFallbackAdjudicationError("Benny fallback answer key has no answers.")
    answer_by_sample = {
        str(row.get("sample_id")): dict(row)
        for row in answers
        if isinstance(row, Mapping) and row.get("sample_id")
    }
    exported_selection = review["selection"]
    selection = _normalize_selection(exported_selection, answer_by_sample)
    ...
    if selection.casefold() == "none":
        project_document["decisions"]["BERNICE"] = {
            "status": "return_to_preparation",
            "primary_method": None,
            "primary_candidate_id": None,
            "summary": (
                "The focused Benny fallback review selected None. Existing specific Benny "
                "routes remain valid, but generalized sardonic concern remains in preparation."
            ),
            ...
            "unresolved_requirements": [
                "Prepare another Benny sardonic-concern reference only if explicitly requested."
            ],
        }
        return {
            "schema_version": 1,
            "round_id": ROUND_ID,
            "exported_selection": exported_selection,
            "selection": "none",
    ...
    selected = answer_by_sample[selection]
    ...
    if not reference_route_key:
        raise BennyFallbackAdjudicationError(
            f"Selected Benny candidate has no reference route: {selection}."
        )

    voice_config = _read_json(project / "voice_config.json", "Voice configuration")
    benny = voice_config.get("BERNICE")
    if not isinstance(benny, Mapping):
        raise BennyFallbackAdjudicationError("BERNICE Voice configuration is missing.")
    policy = benny.get("responsive_backend_routing")
    routes = policy.get("routes") if isinstance(policy, Mapping) else None
    if not isinstance(routes, Mapping):
        raise BennyFallbackAdjudicationError("BERNICE routing policy is missing.")
    reference_route = routes.get(reference_route_key)
    if not isinstance(reference_route, Mapping):
        raise BennyFallbackAdjudicationError(
            f"Selected Benny reference route is missing: {reference_route_key}."
        )
    if (
        ...
        raise BennyFallbackAdjudicationError(
            f"Selected Benny reference route is not strict-approved: {reference_route_key}."
        )
    ...
    project_document["decisions"]["BERNICE"] = {
        ...
        "summary": (
            f"The focused best-or-none review selected {exported_selection}, normalized to "
            f"{selection}. The winning render used the strict-approved {reference_route_key} "
            "Benny identity reference, which now replaces the weaker generalized "
            "sardonic-concern reference."
        ),
    ...
    route_update = {
        ...
        "expected_configuration_fingerprint": str(
            benny.get("responsive_backend_configuration_fingerprint") or ""
        ),
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "output": str(output),
                "exported_selection": result["exported_selection"],
