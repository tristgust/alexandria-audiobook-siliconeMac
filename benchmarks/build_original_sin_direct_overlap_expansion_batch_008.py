#!/usr/bin/env python3
"""Build the final never-attempted strict-overlap batch."""
...
ROUND_ID = "alexandria_original_sin_direct_overlap_expansion_batch_008"
DEFAULT_PLAN = Path(__file__).with_name("original_sin_direct_overlap_expansion_batch_008_plan.json")
DEFAULT_OUTPUT = Path(
    ...
    "direct_overlap_expansion_batch_008"
)
...
def main() -> int:
    ...
    base.REVIEW_HTML = (
        base.REVIEW_HTML
        .replace("Original Sin direct overlap batch 2", "Original Sin direct overlap batch 8")
        .replace("batch 2</h1>", "batch 8</h1>")
    )
    argv = list(sys.argv)
    if "--plan" not in argv:
