#!/usr/bin/env python3
"""Build the final bounded Original Sin character-reference repair round."""
...
ROUND_ID = "alexandria_original_sin_overlap_reference_final_repair_v4"
SEED = 20260731
...
DEFAULT_PLAN = Path(__file__).with_name("original_sin_overlap_reference_final_repair_plan.json")


class FinalRepairError(RuntimeError):
    pass
...
def apply_edge_fades(path: Path, *, fade_out_ms: int = 0) -> None:
    if fade_out_ms <= 0:
        return
    audio, rate = sf.read(str(path), dtype="float32", always_2d=True)
    frames = min(len(audio), max(1, round(rate * fade_out_ms / 1000)))
...
def main() -> int:
    ...
    if plan.get("round_id") != ROUND_ID or sum(len(g["treatments"]) for g in plan["groups"]) != plan.get("candidate_count"):
        raise FinalRepairError("Final repair plan mismatch")
    output = args.output_root.expanduser().resolve() if args.output_root else project / "external_workflows" / "big_finish_overlap_reference_v1" / "reference_final_repair_round_v4"
    if output.exists():
        if not args.replace:
            raise FinalRepairError(f"Output exists; pass --replace: {output}")
    ...
    for spec in plan["groups"]:
        ...
        broad_start = max(0.0, float(segments[start]["start"]) - broad_padding)
        broad_end = float(segments[end]["end"]) + broad_padding
        minimum_end = None
        maximum_end = None
        ...
        slug = re_slug(spec["book_speaker"])
        ...
        alignment = precise_source_cut(
            ...
            word_aliases=spec.get("alignment_word_aliases", {}),
        ...
        for treatment in spec["treatments"]:
            if treatment == "source_mix":
                ...
                path = private / f"{slug}__mel.wav"; separate(vocal, source, path)
            elif treatment == "mossformer2_source_mix":
                path = private / f"{slug}__moss.wav"; enhance(source, path, moss)
            elif treatment == "center_channel_mid":
                path = private / f"{slug}__center.wav"; write_center_channel(source, path)
            else:
                raise FinalRepairError(f"Unsupported treatment: {treatment}")
            apply_edge_fades(path, fade_out_ms=int(spec.get("fade_out_milliseconds", 0)))
            observed = transcribe(path, whisper)
            comparison = transcript_comparison([spec["expected_transcript"]], observed, spec.get("alignment_word_aliases", {}))
            if comparison["word_error_rate"] != 0.0 or not comparison["first_word_present"] or not comparison["last_word_present"]:
                continue
        ...
        if not candidates:
            raise FinalRepairError(f"No objective-eligible final candidate for {spec['character']}")
    ...
    if before != after:
        raise FinalRepairError("Protected project hashes changed")
