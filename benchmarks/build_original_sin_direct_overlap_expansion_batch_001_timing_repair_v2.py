#!/usr/bin/env python3
"""Repair final-phoneme timing for strict-overlap expansion batch 001."""
...
ROUND_ID = "alexandria_original_sin_direct_overlap_expansion_batch_001_timing_repair_v2"
SEED = 20260731
DEFAULT_MEDIA = Path(
...
DEFAULT_PLAN = Path(__file__).with_name(
    "original_sin_direct_overlap_expansion_batch_001_timing_repair_plan.json"
)


class TimingRepairError(RuntimeError):
    pass
...
def build_review(output: Path, groups: list[dict[str, Any]]) -> None:
        ...
        public_groups.append(
            {
                ...
                "transcript": group["transcript"],
                "review_context": (
                    "Timing repair: confirm the complete final phoneme and a natural "
                    "ending. Reject any added sound, adjacent speech, music, or effect."
                ),
    ...
    (review / "data.js").write_text(
        "window.ORIGINAL_SIN_DIRECT_TIMING_REPAIR = "
        + json.dumps(
...
def main() -> int:
    ...
    if plan.get("round_id") != ROUND_ID:
        raise TimingRepairError("Timing-repair plan round mismatch.")
    ...
    if len(selected) != 9 or len(selected) != len(set(selected)):
        raise TimingRepairError("Timing repair must contain nine unique chunks.")
    ...
    output = (
        ...
        "direct_overlap_expansion_batch_001_timing_repair_v2"
    )
    if output.exists():
        if not args.replace:
            raise TimingRepairError(f"Output exists: {output}")
    ...
    for chunk_id in selected:
        ...
        for treatment in plan["treatments"]:
            try:
                if treatment == "mel_roformer_vocal":
                    ...
                    raise TimingRepairError(treatment)
                silence = append_deterministic_silence(
                    wav, int(plan["appended_silence_milliseconds"])
                )
                wav_check = accepted_transcript_check([spec["text"]], transcribe(wav, whisper))
                ...
                if not (
                    ...
                    raise TimingRepairError("Objective transcript/proxy gate failed.")
                candidates.append(
                    {
                        "treatment": treatment,
                        "wav_path": wav,
        ...
        if candidates:
            ...
            print(f"built timing repair {chunk_id} ({len(candidates)} eligible)", flush=True)

    build_review(output, groups)
    after = project_hashes(project)
    if before != after:
        raise TimingRepairError("Protected project hashes changed.")
    write_json(
