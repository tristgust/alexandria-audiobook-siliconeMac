(() => {
  const data = window.THREE_VOICE_OPENVOICE_DATA;
  if (!data || !Array.isArray(data.rows)) throw new Error("Conversion review data is missing.");

  const storageKey = `alexandria:three-voice-openvoice:${data.schema_version}:${data.round_id}`;
  ...
  function renderCards() {
    ...
    visible.forEach((row) => {
      ...
      card.querySelector(".donor-audio").src = row.donor_audio;
      card.querySelector(".converted-audio").src = row.converted_audio;
      const technical = card.querySelector(".technical-status");
      technical.textContent = row.technical_pass ? "Passed automatic stability checks" : "Automatic checks found a concern — listen carefully";
  ...
  function renderNav() {
    const options = [{ key: "all", label: "All 9" }, ...data.target_order.map((key) => ({
      key,
  ...
  function exportReview() {
    ...
    link.download = "alexandria_three_voice_openvoice_conversion_review.json";
