#!/usr/bin/env python3
"""Build the last-mile direct-substitution repair for Powerless and Pryce."""
...
ROUND_ID = "alexandria_original_sin_direct_substitution_last_mile_v6"
...
DEFAULT_PLAN = Path(__file__).with_name("original_sin_direct_substitution_last_mile_plan_v6.json")
...
def provenance(treatment: str) -> dict:
    if treatment == "mel_roformer_blend70":
        base = treatment_provenance("mel_roformer_vocal")
        return {**base, "blend": "70% Mel-RoFormer / 30% source"}
    return treatment_provenance(treatment)
...
def main() -> int:
    ...
    if plan.get("round_id") != ROUND_ID or sum(len(group["treatments"]) for group in plan["groups"]) != 7:
        raise LastMileError("last-mile plan mismatch")
    ...
    for spec in plan["groups"]:
        ...
        if chunk.get("speaker") != spec["book_speaker"] or normalized_words(chunk.get("text")) != normalized_words(spec["expected_transcript"]):
            raise LastMileError(f"chunk binding mismatch: {spec['chunk_id']}")
    output = args.output_root.expanduser().resolve() if args.output_root else project / "external_workflows/big_finish_overlap_reference_v1/direct_substitution_last_mile_round_v6"
    if output.exists():
        if not args.replace:
            raise LastMileError(f"Output exists: {output}")
    ...
    groups = []
    for spec in plan["groups"]:
        start = int(spec["segment_start"]); end = int(spec["segment_end"])
        segment_start = float(segments[start]["start"]); segment_end = float(segments[end]["end"])
        source = private / f"chunk_{spec['chunk_id']}_{re_slug(spec['book_speaker'])}__source.wav"
        alignment = precise_source_cut(
            media=media,
            destination=source,
            broad_path=private / f"chunk_{spec['chunk_id']}__broad.wav",
            ...
            minimum_source_end=segment_end + float(spec["minimum_segment_end_margin_seconds"]),
            maximum_source_end=float(segments[end + 1]["start"]) - float(spec["maximum_next_speaker_margin_seconds"]),
        )
        moss_path = private / f"chunk_{spec['chunk_id']}__moss.wav"
        mel_path = private / f"chunk_{spec['chunk_id']}__mel.wav"
        candidates = []
        for treatment in spec["treatments"]:
            if treatment == "source_mix":
                ...
                wav = moss_path
                if not wav.exists(): enhance(source, wav, moss)
            elif treatment == "mel_roformer_vocal":
                wav = mel_path
                if not wav.exists(): separate(vocal, source, wav)
            elif treatment == "mossformer2_blend50":
                if not moss_path.exists(): enhance(source, moss_path, moss)
                wav = private / f"chunk_{spec['chunk_id']}__moss_blend50.wav"
                write_blend(source, moss_path, wav, 0.50)
            elif treatment == "mel_roformer_blend70":
                if not mel_path.exists(): separate(vocal, source, mel_path)
                wav = private / f"chunk_{spec['chunk_id']}__mel_blend70.wav"
                write_blend(source, mel_path, wav, 0.70)
            else:
                raise LastMileError(treatment)
            wav_check = accepted_transcript_check([spec["expected_transcript"]], transcribe(wav, whisper))
            proxy = private / f"chunk_{spec['chunk_id']}__{treatment}.mp3"
            encode_proxy(wav, proxy, bitrate=plan["production_proxy"]["bitrate"])
            proxy_check = accepted_transcript_check([spec["expected_transcript"]], transcribe(proxy, whisper))
        ...
        if not candidates:
            raise LastMileError(f"No eligible candidates: {spec['character']}")
        groups.append({
            "character": spec["character"], "book_speaker": spec["book_speaker"],
            "chunk_id": int(spec["chunk_id"]), "transcript": spec["expected_transcript"],
            "review_context": spec.get("review_context"),
            "source": {"media_sha256": sha256_file(media), "segment_start": start, "segment_end": end, **alignment},
    ...
    if before != after:
        raise LastMileError("protected project hashes changed")
    write_json(output / "generation-summary.json", {
        "schema_version": 1, "round_id": ROUND_ID, "generated_at": utc_now(),
        "chunk_count": len(groups), "candidate_count": sum(len(group["candidates"]) for group in groups),
