...
const INTERFACE_LABEL = 'new-writable';
...
function serveStatic(request, response, staticRoot) {
  ...
  response.writeHead(200, {
    ...
    'X-Alexandria-Interface': INTERFACE_LABEL,
    'X-Alexandria-Mutations': 'enabled',
  });
...
function proxyRequest(request, response, upstream) {
  ...
  proxy.on('error', (error) => {
    ...
    else response.writeHead(502, { 'Content-Type': 'text/plain; charset=utf-8' })
      .end(`New Alexandria interface backend unavailable: ${error.message}`);
  });
  request.pipe(proxy);
}
...
async function main() {
  ...
  const staticRoot = path.resolve(String(args['static-root'] || path.join(ROOT, 'app', 'static')));
  if (!Number.isInteger(port) || port < 0 || port > 65535) {
    throw new Error('--port must be an integer from 0 through 65535');
  }
  if (!fs.existsSync(path.join(staticRoot, 'index.html'))) {
    throw new Error(`New UI static root is missing index.html: ${staticRoot}`);
  }
  await waitForUpstream(upstream);

  const server = http.createServer((request, response) => {
    if (request.url === '/__alexandria_new_ui__') {
      response.writeHead(200, {
  ...
  process.stdout.write(`Alexandria new writable interface: ${url}\n`);
...
main().catch((error) => {
  process.stderr.write(`Alexandria new writable interface failed: ${error.message}\n`);
  process.exitCode = 1;
});
